...
+ (WMAssetWriter*)encoderForPath:(NSString*) path Height:(NSInteger) cy width:(NSInteger) cx channels: (int) ch samples:(Float64) rate videoBitRate:(NSUInteger)videoBitRate audioBitRate:(NSUInteger)audioBitRate{
    WMAssetWriter* enc = [WMAssetWriter new];
    return [enc initPath:path Height:cy width:cx channels:ch samples:rate videoBitRate:videoBitRate audioBitRate:audioBitRate];
}
...
- (instancetype)initPath:(NSString*)path Height:(NSInteger)cy width:(NSInteger)cx channels:(int)ch samples:(Float64) rate videoBitRate:(NSUInteger)videoBitRate audioBitRate:(NSUInteger)audioBitRate{
    ...
    if (self) {
        ...
        //初始化视频输入
        [self initVideoInputHeight:cy width:cx videoBitRate:videoBitRate];
        //确保采集到rate和ch
        if (rate != 0 && ch != 0) {
            //初始化音频输入
            [self initAudioInputChannels:ch samples:rate audioBitRate:audioBitRate];
        }
    }
    return self;
}
...
//初始化视频输入
- (void)initVideoInputHeight:(NSInteger)cy width:(NSInteger)cx videoBitRate:(NSUInteger)bitRate{
...
//初始化音频输入
- (void)initAudioInputChannels:(int)ch samples:(Float64)rate audioBitRate:(NSUInteger)bitRate{
    //音频的一些配置包括音频各种这里为AAC,音频通道、采样率和音频的比特率
    NSDictionary *settings = [NSDictionary dictionaryWithObjectsAndKeys:
                              [ NSNumber numberWithInt: kAudioFormatMPEG4AAC], AVFormatIDKey,
                              [ NSNumber numberWithInt: ch], AVNumberOfChannelsKey,
                              [ NSNumber numberWithFloat: rate], AVSampleRateKey,
                              [ NSNumber numberWithInteger: bitRate], AVEncoderBitRateKey,
                              nil];
    //初始化音频写入类
...
- (BOOL)encodeFrame:(CMSampleBufferRef)sampleBuffer isVideo:(BOOL)isVideo {
    ...
    if (CMSampleBufferDataIsReady(sampleBuffer)) {
        ...
        if (_writer.status == AVAssetWriterStatusFailed) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"writeVideoDataError" object:_writer.error.localizedDescription];
            return NO;
        }
        //判断是否是视频
        if (isVideo) {
